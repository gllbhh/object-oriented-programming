...
class Refrigerator {
	...
	getAndEatFood(name) {
		...
		let index = this.findFoodInTheFridge(name);
		if (index === null) {
			console.log("Sorry, no such food in this refrigerator!");
			return;
		}
		if (this.storage[index].quantity > 0) {
			this.storage[index].quantity--;
			console.log(
				`Slurp! One ${this.storage[index].name} eaten. ${this.storage[index].quantity} remaning`
			);
		} else {
			console.log("Sorry, no such food in this refrigerator!");
		}
